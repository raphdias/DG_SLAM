...
def pose_evaluation(poses_gt, poses_est, path, idx):
    N = len(poses_est)
    scale = 1
    poses_est, _ = convert_poses(poses_est, N, scale)
    plt_path = os.path.join(path, 'pose_{}.png'.format(idx))

    results = evaluate(poses_gt, poses_est, plot=plt_path)
    results['Name'] = idx
    print(results, file=open(os.path.join(path, 'output.txt'), "a"))
    return results

def pose_evaluation_rel(poses_gt, poses_est, path, idx):
    N = len(poses_est)
    scale = 1
    poses_gt, mask = convert_poses(poses_gt, N, scale)
    poses_est, _ = convert_poses(poses_est, N, scale)
    poses_est = poses_est[mask]
    plt_path = os.path.join(path, 'pose_rel{}.png'.format(idx))
