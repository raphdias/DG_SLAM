	...
	const bool debug)
{
  ...
  return std::make_tuple(rendered, out_color, out_depth, out_acc, radii, geomBuffer, binningBuffer, imgBuffer);
}
	...
	const bool debug) 
{
  ...
  {  
	  ...
	  dL_ddepthi.contiguous().data<float>(),
  ...
  return std::make_tuple(dL_dmeans2D, dL_dcolors, dL_dopacity, dL_dmeans3D, dL_dcov3D, dL_dsh, dL_dscales, dL_drotations, dL_dviewmatrix, dL_dcampos);
